...
void APortal::BeginPlay()
{
	...
	SetTickGroup(ETickingGroup::TG_PostUpdateWork);
	PortalMat = UKismetMaterialLibrary::CreateDynamicMaterialInstance(GetWorld(), PortalMatReference);
...
void APortal::SetClipPlane()
{
	FVector Forward = NormalVisible.TransformVector(FVector::ForwardVector);

	PortalCamera->bEnableClipPlane = true;
	PortalCamera->ClipPlaneBase = RenderPlane->GetComponentLocation() * Forward;
	PortalCamera->ClipPlaneNormal = Forward;
}

void APortal::CheckViewportSize()
{
...
void APortal::RenderView(UCameraComponent* ViewCamera, const FVector RefPosition, const FQuat RefRotation)
{
	...
	SetProjectionMatrix(ViewCamera);
	SetClipPlane();
}
